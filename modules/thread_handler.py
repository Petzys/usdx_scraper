...
def thread_runner_txt(cookie_list:list, payload:str, download_url:str, user_args:dict, index:int, target:list):
    ...
    for count, cookie in enumerate(cookie_list):
        cli_handler.status_print(iterator=cookie_list, count=count, content=f'Downloading .txt files with cookie = {cookie[:-1]}')
        ...
        try:
            ...
            if not folder in folder_list:
                ...
                cli_handler.status_print(iterator=cookie_list, count=count, content=f'This song already exists, skipping...')
                folder_list.append(None)
        except ConnectionError or requests.exceptions.RetryError:
            cli_handler.status_print(iterator=cookie_list, count=count, content=f'Error while downloading .txt files, skipping {cookie[:-1]}...')
            folder_list.append(None)
...
def thread_runner_download(song_folder_tuples:tuple, user_args:dict): 
    ...
    for count, (song, folder) in enumerate(song_folder_tuples):
        try: 
            cli_handler.status_print(iterator=song_folder_tuples, count=count, content=f'Getting YT URL for song {song[1]}')
            url = youtube_handler.get_yt_url(song=song[1], id=song[0])

            cli_handler.status_print(iterator=song_folder_tuples, count=count, content=f'Downloading {song[1]}')
            folder = youtube_handler.download_song(song=song[1], folder=folder, songs_directory=user_args["output_path"], url=url)

            cli_handler.status_print(iterator=song_folder_tuples, count=count, content=f'Cleaning up filenames and references in {folder}')
            txt_files_handler.clean_tags(songs_directory=user_args["output_path"], song_folder=folder)
        
        except Exception as e:
            cli_handler.status_print(iterator=song_folder_tuples, count=count, content=f'Error while getting stream or downloading, skipping and deleting shallow folder...')
